...
function M:init(opts)
  require("user.util.lazyterm.colors").setup()
...
function M:mount()
  ...
  local function opts()
    ...
    M.wo(self.win, "conceallevel", 3)
    M.wo(self.win, "foldenable", false)
    M.wo(self.win, "spell", false)
    M.wo(self.win, "wrap", true)
    M.wo(self.win, "winhighlight", "Normal:LazyNormal")
    M.wo(self.win, "colorcolumn", "")
  end
  opts()
...
function M:close(opts)
  ...
  vim.schedule(function()
    ...
    if wipe and buf and vim.api.nvim_buf_is_valid(buf) then
      vim.diagnostic.reset(vim.api.nvim_create_namespace("lazy"), buf)
